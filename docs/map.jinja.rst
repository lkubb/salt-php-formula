...
We will see a quick setup to configure the ``php`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``php`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``php`` formulas under ``/srv/salt/php/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/php/parameters/dns:domain/
    ln -s dns:domain /srv/salt/php/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/php/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/php/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``php`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``php`` formulas under ``/srv/salt/php/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/php/parameters/roles
...
We create a configuration for the role ``php/server`` in ``/srv/salt/php/parameters/roles/php/server.yaml``:
...
We create another configuration for the role ``php/client`` in ``/srv/salt/php/parameters/roles/php/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles php/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles php/client
...
.. code-block:: console
    ...
    mv /srv/salt/php/parameters/dns:domain/  '/srv/salt/php/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-php-as-json:
      test.nop:
        - name: {{ php | json }}
...
Here is an example based on `template-formula/php/config/file.sls`_:
    ...
    php-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='php-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
...
This ``sls`` file expose a ``php`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ php['winner'] }}
...
.. _template-formula/php/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/php/config/file.sls
